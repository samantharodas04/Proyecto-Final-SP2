...
namespace CreditosApi.Services
{
    public class AwsRekognitionService
    {
      ...
      // Verifica que el número de DPI aparezca en el texto del DPI
      public async Task<bool> ValidarTextoDpi(byte[] dpiFoto, string dpiIngresado)
      {
        ...
        return response.TextDetections.Any(t =>
            t.DetectedText != null &&
            t.DetectedText.Replace(" ", "")
              .Contains(dpiIngresado.Replace(" ", ""))
        );
      }
    }
}
